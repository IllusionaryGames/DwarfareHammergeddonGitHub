...
public class EditorPrefsX
{
	...
	private static bool SetValue<T> (String key, T array, EditorArrayType arrayType, int vectorNumber, Action<T, byte[],int> convert) where T : IList
	{
		if (array.Count == 0)
		{
	...
	private static void GetValue<T> (String key, T list, EditorArrayType arrayType, int vectorNumber, Action<T, byte[]> convert) where T : IList
	{
		if (EditorPrefs.HasKey(key))
		{
			var bytes = System.Convert.FromBase64String (EditorPrefs.GetString(key));
			...
			if ((EditorArrayType)bytes[0] != arrayType)
			{
	...
	public static void ShowArrayType (String key)
	{
		...
		if (bytes.Length > 0)
		{
			EditorArrayType arrayType = (EditorArrayType)bytes[0];
